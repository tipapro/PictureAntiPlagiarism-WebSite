...
class ImageIterator:
    ...
    def __iter__(self):
        ...
        self.__task__ = asyncio.ensure_future(self.__download_next_arrs__(self.__cur_pos__ + self.__chunk_size__))
        return self
    ...
    def __next__(self):
        ...
        if self.__cur_pos__ < self.count:
            if self.__cur_arr_pos__ < self.__chunk_size__:
                image_ids, arr = self.__cur_arr_vec__[self.__cur_arr_pos__], self.__cur_arr_id__[self.__cur_arr_pos__]
                self.__cur_arr_pos__ += 1
                self.__cur_pos__ += 1
                return image_ids, arr
            else:
                self.__loop__.run_until_complete(self.__task__)
    ...
    async def __download_next_arrs__(self, start):
